...
class Populacao{
    ...
    private char letra[] = {'a','b','c','d','e','f','g','h','i','j','k','l','m','n','o','p','q','r','s','t','u','v','w','x','y','z',' '}; //Caracteres auxiliares
...
//Construtor que cria um array com vários elementos aleatórios
    public Populacao(int quantidade, String palavraDesejada) 
    {
        for(int i = 0; i < quantidade; i++)
        {
            ...
            int aux;
            StringBuilder auxiliar = new StringBuilder(); //Cria uma string auxiliar que irá formar as palavras aleatórias
...
//Printa todos os elementos da população na tela
    public void printa_elementos()
    {
        ...
        for (int i = 0; i < element.size(); i++) {
            System.out.println("ELEMENTO: " + element.get(i).getDna() + "  || FITNESS: " + element.get(i).getFitness());
        ...
        {
            System.out.println("****************************************************");
            System.out.println("MELHOR ELEMENTO: " + element.get(aux).getDna() + "  || FITNESS: " + element.get(aux).getFitness());
            System.out.println("****************************************************");
        }
        media_fit();
    }
//Método para calcula a média de adequação da população total
    public void media_fit()
    {
        ...
        System.out.println("*******************************" );
        System.out.println("Media de Acerto: " + this.mediaFit);
        System.out.println("********************************");
    }
...
//Gera um novo arraylist levando em conta as proporções do fitness. Quanto maior o fitness, mais vezes aparece o elemento
    public void natural_selection(int quantidade, String palavraDesejada)
    {
        for(int i = 0; i < quantidade; i++)
        {
            ...
            for(int n = 0; n < aux; n++) //Repete o elemento N vezes dentro do novo arraylist, sendo N proporcional ao fitness do elemento
            {
...
//Função que faz o crossover do pai e da mae
    ...
    {
        char pai[] = (matingPool.get(auxPai).getDna()).toCharArray(); //Transforma o nome do pai e da mae em um array de char
        ...
        if(matingPool.get(auxMae).getFitness() >= matingPool.get(auxPai).getFitness()) //Se o fitness da mae for maior, ela passa mais genes aos filhos
        {
            for(int i = 0; i < med; i++) 
            {
        ...
        else if(matingPool.get(auxPai).getFitness() > matingPool.get(auxMae).getFitness()) //Se o fitness do pai for maior, ele passa mais genes aos filhos
        {
            for(int i = 0; i < med; i++) 
            {
...
//Função que realiza a mutação
    public void mutate(Elemento temp, double rate)
    {
        ...
        //Cria um vetor que irá auxiliar no uso da portentagem escolhida. Ele valerá 1 "rate%" vezes
        int probabilidades[];
        ...
        for(int i = 0; i < aux.length; i++)
        {
            ...
            if(probabilidades[indice] == 1) //Se valer 1, realiza uma mutação, escolhendo um novo valor para o caractere
            {
...
//Função que gera o loop evolutivo(função recursiva)
    public void evolui(int quantidade, String palavraDesejada)
    {
        ...
        while(gen <= 2000)
        {
           ...
           // System.out.println("*********");
            limpa_tela();
            try { Thread.sleep (15); } catch (InterruptedException ex) {} //Dá uma pausa de 0.8 Segundo
            ...
            matingPool.clear();
            //Até aqui, todos os novos elementos estão no arraylist filho. Vamos passa-los para o arraylist principal
            element.clear();
